...
struct cmos_map volteer_cmos_map[] = {
	{ CMOS_DEVICE_PCH, "LPSS0", 0, 128, 0x29, coreboot_cmos_bank0_vars },
	{ CMOS_DEVICE_PCH, "LPSS1", 1, 128, 0x00, coreboot_cmos_bank1_vars },
};

static const uint16_t volteer_cmos_port[] = { 0x70, 0x72 };

static uint8_t volteer_read_cmos(struct platform_intf *intf,
				 int addr, int reg)
{
	uint8_t data;

	io_write8(intf, volteer_cmos_port[addr], reg);
	io_read8(intf, volteer_cmos_port[addr] + 1, &data);
	return data;
}

static void volteer_write_cmos(struct platform_intf *intf,
			       int addr, int reg, uint8_t val)
{
	io_write8(intf, volteer_cmos_port[addr], reg);
	io_write8(intf, volteer_cmos_port[addr] + 1, val);
}

static int volteer_nvram_list_bank(struct platform_intf *intf,
				  struct cmos_map *map)
{
...
static int volteer_nvram_list(struct platform_intf *intf)
{
	int dev, rc = 0;

	/* handle each cmos bank */
	for (dev = 0; dev < ARRAY_SIZE(volteer_cmos_map); dev++)
		rc |= volteer_nvram_list_bank(intf, &volteer_cmos_map[dev]);

	return rc;
}

static int volteer_nvram_dump(struct platform_intf *intf)
{
	struct cmos_map *map;
	int off, dev;
	...
	for (dev = 0; dev < ARRAY_SIZE(volteer_cmos_map); dev++) {
		...
		case CMOS_DEVICE_PCH:
			for (off = 0; off < map->length; off++)
				cmos_data[off] = volteer_read_cmos(
					intf, map->bank, off);
			break;
		}
...
static int volteer_nvram_clear(struct platform_intf *intf)
{
	struct cmos_map *map;
	int off, dev;
	...
	/* handle each cmos bank */
	for (dev = 0;
	     dev < (sizeof(volteer_cmos_map) / sizeof(struct cmos_map));
	     dev++) {
		map = &volteer_cmos_map[dev];
		...
		case CMOS_DEVICE_PCH:
			for (off = map->clear_start; off < map->length; off++)
				volteer_write_cmos(intf, map->bank, off, 0x00);
			break;
		}
	}
...
struct nvram_cb volteer_nvram_cb = {
	.list		= volteer_nvram_list,
	.dump		= volteer_nvram_dump,
	.clear		= volteer_nvram_clear,
